...
tags:
  [
    트리 트리구조 tree structure 계층 인접 목록 모델 중첩 세트 adjancency list nested set model,
  ]
---
...
다음과 같은 쿼리를 사용하면 우리가 원하는 구조로 JSON데이터를 반환합니다. `"sub_cat"` 키는 조인을 사용할때 참조하는 테이블 별명을 반영하더라구요. `LEFT JOIN` 절에 있는 Categories 테이블은 `sub_cat`으로 별명을 정했습니다.
