...
categories: Database
comments: true
tags: [트리 트리구조 tree structure 계층]
---
...
```sql
SELECT p.prod_id, p.prod_name, p.prod_price, cat.cat_id, cat.cat_name
FROM Products AS p
LEFT OUTER JOIN Categories AS cat
ON p.cat_id = cat.cat_id
```

쿼리를 실행하면 개발환경에서 JSON데이터로 뽑아오면 다음과 같은 데이터가 나올겁니다.
